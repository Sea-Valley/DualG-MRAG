...
from gfmrag.ultra.explain import normalize_edge_messages_to_flow
...
class GeneralizedRelationalConv(MessagePassing):
    ...
    def forward(
        ...
        trace = self.compute_local_flow(
            input=input,
            relation=relation,
            target_node_states=output,
            edge_index=edge_index,
            edge_type=edge_type,
    ...
    def compute_edge_messages(self, input, relation, edge_index, edge_type):
        source_index = edge_index[0]
        input_j = input.index_select(self.node_dim, source_index)
        relation_j = relation.index_select(self.node_dim, edge_type)

        if self.message_func == "transe":
            return input_j + relation_j
        if self.message_func == "distmult":
            return input_j * relation_j
        if self.message_func == "rotate":
    ...
    def compute_local_flow(
        ...
        edge_messages = self.compute_edge_messages(input, relation, edge_index, edge_type)
        flow_trace = normalize_edge_messages_to_flow(
            edge_messages=edge_messages,
            target_node_states=target_node_states,
            edge_index=edge_index,
            temperature=temperature,
            eps=eps,
        )
        flow_trace["edge_index"] = edge_index
        flow_trace["edge_type"] = edge_type
        return flow_trace
