...
class EMF1Evaluator(BaseEvaluator):
    """Generic exact-match / token-F1 evaluator for short-form QA."""

    def evaluate(self) -> dict:
        metrics = {"em": 0.0, "f1": 0.0, "precision": 0.0, "recall": 0.0}

        for pred in self.data:
            pre_ans = str(pred.get("pred_answer") or "").strip()
            if not pre_ans:
                response = pred["response"]
                if "Answer: " in response:
                    pre_ans = response.split("Answer:")[1].strip()
                else:
                    pre_ans = response
