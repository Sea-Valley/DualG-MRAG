...
class GNNRetriever(QueryGNN):
    ...
    def explain_forward_paths(
        ...
        entities_weight: torch.Tensor | None = None,
        target_topk: int = 5,
        target_selection_mode: str = "score",
        target_score_topk: int | None = None,
        target_mass_topk: int | None = None,
        ...
        assert batch_size == 1, (
            "Currently only supports batch size 1 for forward path explanation"
        )
        ...
        return self.entity_model.explain_forward_paths(
            graph,
            sample,
            input,
            relation_representations,
            question_embedding,  # type: ignore
            source_weights=question_entities_mask,
            target_topk=target_topk,
            target_selection_mode=target_selection_mode,
            target_score_topk=target_score_topk,
            target_mass_topk=target_mass_topk,
